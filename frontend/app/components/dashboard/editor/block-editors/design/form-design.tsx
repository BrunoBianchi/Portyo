...
export const FormDesignEditor = memo(function FormDesignEditor({ block, onUpdate }: Props) {
  return (
      ...
      <EditorSection icon={<Palette className="w-3.5 h-3.5" />} title="Cores" noBorder>
        <div className="grid grid-cols-2 gap-3">
          <EditorColorField
            ...
            onChange={(v) => onUpdate({ formBackgroundColor: v })}
          />
          <EditorColorField
            label="Cor do texto"
            value={block.formTextColor || "#1f2937"}
            onChange={(v) => onUpdate({ formTextColor: v })}
          />
        </div>
      </EditorSection>
    </div>
