...
export const FeaturedDesignEditor = memo(function FeaturedDesignEditor({ block, onUpdate }: Props) {
  return (
      ...
      <EditorSection icon={<Palette className="w-3.5 h-3.5" />} title="Cores" noBorder>
        <div className="grid grid-cols-2 gap-3">
          <EditorColorField
            ...
            onChange={(v) => onUpdate({ featuredColor: v })}
          />
          <EditorColorField
            label="Cor do texto"
            value={block.featuredTextColor || "#ffffff"}
            onChange={(v) => onUpdate({ featuredTextColor: v })}
          />
        </div>
      </EditorSection>
    </div>
