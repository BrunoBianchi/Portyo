...
export const BlogPostPopup: React.FC<BlogPostPopupProps> = ({ post, onClose, config }) => {
  ...
  const MarkdownContent = () => (
    <div className="prose prose-lg max-w-none prose-headings:font-bold prose-a:text-primary hover:prose-a:text-primary/80 prose-img:rounded-xl">
      <ReactMarkdown
          components={{
              p: ({node, ...props}) => <p style={{color: textColor, opacity: 0.9}} {...props} />,
              h1: ({node, ...props}) => <h1 style={{color: textColor}} {...props} />,
              h2: ({node, ...props}) => <h2 style={{color: textColor}} {...props} />,
              h3: ({node, ...props}) => <h3 style={{color: textColor}} {...props} />,
              li: ({node, ...props}) => <li style={{color: textColor, opacity: 0.9}} {...props} />,
  ...
  if (style === "modern") {
    return (
      ...
      >
        <div 
          className="relative w-full max-w-5xl h-[85vh] bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in zoom-in-95 duration-300"
  ...
  if (style === "simple") {
    return (
      ...
      >
        <div 
          className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl shadow-xl animate-in slide-in-from-bottom-4 duration-300 border border-gray-100"
          style={{ backgroundColor: bgColor, color: textColor }}
          onClick={(e) => e.stopPropagation()}
        >
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200"
        style={{ backgroundColor: bgColor, color: textColor }}
        onClick={(e) => e.stopPropagation()}
      >
